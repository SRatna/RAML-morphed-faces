...
def image_encoding(model, facepaths):
    ...
    for c, imgs in enumerate(imgchunks):
        ...
        print(np.argmax(model(torch.Tensor(im_array.transpose(0, 3, 1, 2)))[0].detach().cpu().numpy()))
        start = c * batch_size
